...
public Action:clientlib_ShowHudText(Handle:timer, any:data)
{
	...
	for (new client = 1; client <= MaxClients; client++)
	{
		...
		if (clientlib_isValidClient(client))
		{
			...
			// Show the hud text
			SetHudTextParams(startPos, endPos, 0.6, 109, 132, 180, 255, 0, 0.0, 0.0, 0.0);
			ShowSyncHudText(client, g_hHudSync, "Punkte: %i Level: %s", g_iPlayerPoints[client], g_sLevelName[g_iPlayerLevel[client]-1]);
		}
	}
