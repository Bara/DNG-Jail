...
public OnPluginStart()
{
	...
	gH_Cvar_FruitNinja_M1Time = 		CreateConVar("sm_fn_m1_time", "30", "Mode 1's (normal) long.", 0, true, 20.0, true, 120.0);
	FruitNinja_M1Time = 30;
	gH_Cvar_FruitNinja_M2Time = 		CreateConVar("sm_fn_m2_time", "30", "Mode 2's (iPhone like) long.", 0, true, 20.0, true, 120.0);
	FruitNinja_M2Time = 30;
		...
		if ( IsPlayerAlive( Client ) )
		{
			for ( new i = 0; i < GetRandomInt( 3, 10 ); i++ )
			{
				...
				if(IsValidEntity(Prop))
					CreateTimer(2.0, Timer_DeleteProp, EntIndexToEntRef(Prop));
			}
		}
	}
}

public Action Timer_DeleteProp(Handle timer, any refIndex)
{
	int Prop = EntRefToEntIndex(refIndex);
