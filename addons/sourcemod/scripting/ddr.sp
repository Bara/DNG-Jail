...
public void OnMapStart()
{
    ...
    if (!IsValidMap(sMap))
    {
        SetFailState("%s doesn't support DDR.", sMap);
        return;
    }
...
bool IsValidMap(const char[] sMap)
{
    char sFile[PLATFORM_MAX_PATH + 1];
    BuildPath(Path_SM, sFile, sizeof(sFile), "%s/maps.ini", PATH_CONFIG);

    File fFile = OpenFile(sFile, "rt");

    if (fFile == null)
    {
        SetFailState("[DDR] Can't open File: %s", sFile);
    }
    ...
    while (!fFile.EndOfFile() && fFile.ReadLine(sLine, sizeof(sLine)))
    {
        ...
        if (strlen(sLine) > 1)
        {
            if (StrContains(sMap, sLine, false) != -1)
            {
                bValid = true;
                break;
            }
