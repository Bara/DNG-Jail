...
public void OnPluginStart()
{
	...
	RegAdminCmd("sm_psay", Command_SmPsay, ADMFLAG_CUSTOM4, "sm_psay <name or #userid> <message> - sends private message");
...
public Action OnClientSayCommand(int client, const char[] command, const char[] sArgs)
{
	...
	if (strcmp(command, "say", false) == 0)
	{
		...
		{
			if (!CheckCommandAccess(client, "sm_psay", ADMFLAG_CUSTOM4))
			{
				return Plugin_Continue;
			}
...
public Action Command_SmMsay(int client, int args)
{
	...
	if (client > 0 && SourceComms_GetClientGagType(client) != bNot)
		return Plugin_Handled;
...
void SendChatToAll(int client, const char[] message)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		CPrintToChat(i, "{darkred}(ALLE) {orchid}%s: {lightblue}%s", nameBuf, message);
	}
}
...
{
	if (SourceComms_GetClientGagType(from) != bNot)
		return;
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		{
			CPrintToChat(i, "{darkred}(%sADMINS) {orchid}%N: {lightgreen}%s", fromAdmin ? "" : "TO ", from, message);
		}	
	}
}
