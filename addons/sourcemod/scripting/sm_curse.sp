...
public void OnPluginStart() {
	...
	g_cvar_jump = CreateConVar("sm_curse_invert_jump", "1.0", "Invert movement while jumping (duck & jump)", FCVAR_NONE, true, 0.0, true, 1.0);
...
public int MenuHandler_CurseVote(Menu menu, MenuAction action, int param1, int param2) {
	if(action == MenuAction_VoteEnd) {
		...
		float percent = float(votes) / float(total_votes);
