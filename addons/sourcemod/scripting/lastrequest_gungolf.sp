		...
		if(param2 == 0)
		{
			if((GetEntityFlags(param1) & FL_ONGROUND))
			{
				if(!IsClientTooNearObstacle(param1))
				{
					...
					iKnife = -1;
					iKnife = GivePlayerItem(g_LR_Player_Guard, "weapon_knife");
					EquipPlayerWeapon(g_LR_Player_Guard, iKnife);
					...
					CPrintToChatAll("%t", "GG Start", g_LR_Player_Prisoner, g_LR_Player_Guard);
					InitializeLR(param1);
				}
				else
				{
					CPrintToChat(param1, "%t", "Obstacle");
					DisplayMenu(g_PointSelect, param1, 0);
				}
			}
			else
			{
				CPrintToChat(param1, "%t", "On Ground");
				DisplayMenu(g_PointSelect, param1, 0);
			}
		}
	}
...
{
	if(GetArrayCell(LR_Array, iIndexInArray, _:Block_LRType) != 0)
	{
	...
	if(Type == g_LREntryNum)
	{
		if (IsClientInGame(Prisoner) && IsClientInGame(Guard))
		{
			if (IsPlayerAlive(Prisoner) && IsPlayerAlive(Guard))
			{
				...
				iKnife = GivePlayerItem(Guard, "weapon_knife");
				EquipPlayerWeapon(Guard, iKnife);
				CPrintToChatAll("%t", "GG Stopped");
			}
			else if(IsPlayerAlive(Prisoner))
			{
				...
				int iKnife = GivePlayerItem(Prisoner, "weapon_knife");
				EquipPlayerWeapon(Prisoner, iKnife);
				CPrintToChatAll("%t", "GG Winner", Prisoner);
			}
			else if (IsPlayerAlive(Guard))
			{
				...
				EquipPlayerWeapon(Guard, iKnife);
				CPrintToChatAll("%t", "GG Winner", Guard);
			}
		}
