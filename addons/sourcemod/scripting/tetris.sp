...
#define PREFIX "{darkred}[Tetris]{green} "
...
// Game panel
new Handle:g_hPlayerGameThink[MAXPLAYERS+1] = {null,...};
...
new Handle:g_hDrawInfoPanel[MAXPLAYERS+1] = {null,...};
new Handle:g_hPlayBackgroundMusic[MAXPLAYERS+1] = {null,...};
...
public OnPluginStart()
{
	...
	AutoExecConfig_SetFile("plugin.tetris");
	g_hCVDisableHardDrop = AutoExecConfig_CreateConVar("tetris_disableharddrop", "0", "Disable hard dropping the tetrimino instantly to the floor with the spacebar?", _, true, 0.0, true, 1.0);
	g_hCVDisableSounds = AutoExecConfig_CreateConVar("tetris_disablesounds", "0", "Disable all tetris sounds?", _, true, 0.0, true, 1.0);
	g_hCVOnlyDead = AutoExecConfig_CreateConVar("tetris_onlydead", "0", "Player has to be dead to play tetris?", _, true, 0.0, true, 1.0);
	g_hCVDisableStats = AutoExecConfig_CreateConVar("tetris_disableinfopanel", "0", "Disable showing stats like score/cleared bars in a (Key)Hint panel while playing?", _, true, 0.0, true, 1.0);
	g_cMax = AutoExecConfig_CreateConVar("tetris_max_games", "2", "Max games pro dead (normal player)", _, true, 2.0);
	g_cMaxVIP = AutoExecConfig_CreateConVar("tetris_max_games_vip", "4", "Max games pro dead (vip player)", _, true, 4.0);
	...
	Database.Connect(SQL_OnDatabaseConnected, "deadgames");
...
public Event_OnPlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(g_hPlayerGameThink[client] != null)
	{
		...
		if (!ValidTeam(client))
		{
			...
			CPrintToChat(client, "{darkred}[Snake] {green}Dies geht nur als {orchid}Toter!", PREFIX);
			return;
		}
...
DisplayTetrisMainMenu(client)
{
	...
	if(g_iObjectType[client] != Tetris_None && ValidTeam(client))
...
public SQL_OnDatabaseConnected(Database db, const char[] error, any data)
{
	...
	{
		SQL_TQuery(db, SQL_DoNothing, "CREATE TABLE IF NOT EXISTS tetris_players (steamid VARCHAR(64) PRIMARY KEY, name VARCHAR(64) NOT NULL, score_easy INTEGER DEFAULT '0', score_normal INTEGER DEFAULT '0', score_hard INTEGER DEFAULT '0');");
	}
	else
	{
...
public SQL_FetchTop5(Handle:owner, Handle:hndl, const String:error[], DataPack data)
{
	new userid, difficulty;
	if(data != null)
	{
		data.Reset();
		userid = data.ReadCell();
		difficulty = data.ReadCell();
		delete data;
	}
...
bool ValidTeam(int client)
{
	if(g_bOnlyDead && IsPlayerAlive(client) || GetClientTeam(client) < 2)
		return false;
